<?php require_once("../conn.php"); 
    ...
    $show = mysqli_query($conn, "SELECT * FROM tujuan_toba_wisata WHERE id_tujuan='$id'");
      
  if(mysqli_num_rows($show)==0){
    // echo '<script>window.history.back()</script>';
  }else{
    $tujuans = mysqli_fetch_array($show);
      }
...
font-size: 16px;"> <a href="../logout.php" class="btn btn-success square-btn-adjust">Logout</a> </div>
           ...
           <!-- /. NAV TOP  -->
            ...
            <div class="sidebar-collapse">
				...
				<!-- <li class="text-center">
                    ...
                    <li>
                        <a  class="active-menu" href="#"><i class="fa fa-exchange fa-3x"></i> Daftar Tujuan</a>
     ...
     <div class="container">	
		<div class="row">
			<div class="col-md-10">
				<h3>Perbarui Tujuan Paket Wisata</h3>

        <form action="perbarui_tujuan_toba_wisata_process.php?id_tujuan=<?=$id?>" method = "post" enctype="multipart/form-data">

				<div class="col-sm-8">
          <label>Tujuan Wisata</label>
          <input class="form-control" type="text" name="tempatWisata" placeholder="<?=$tujuans['tempat_wisata']?>"><br>
          <label>Harga</label>
          <input class="form-control"  type="number" name="harga" placeholder="<?=number_format($tujuans['harga'])?>"><br>

          <button class="btn btn-primary" name="perbaruiTujuan">Perbarui</button>
        </form>
        </div>
			</div>
		</div>
